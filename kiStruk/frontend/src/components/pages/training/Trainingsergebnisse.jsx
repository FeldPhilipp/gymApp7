...
// ============================================================
// UebungCard – Dropsätze leben in ergebnisse[uiId].dropsaetze[]
// ============================================================
function UebungCard({
  uebung,
  onEdit,
  ...
  onChange,             // (uiId, satzIdx, field, value)
      ...
      <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between" mb={2}>
          <Box display="flex" alignItems="center" gap={1} flex={1}>
            ...
            <Box flex={1}>
              ...
              <ExerciseChips zielmuskel={uebung.zielmuskel} kategorie={uebung.kategorie} />
            </Box>
          </Box>
        ...
        <Collapse in={expanded}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {/* ── Normale Sätze ── */}
            <ExerciseSetInputs exerciseId={uebung.uiId} sets={saetze} onChange={onChange} />
            {false && saetze.length > 0 ? saetze.map((satz, satzIdx) => (
            )) : (
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 6 }}>
                      <TextField
                        ...
                        onChange={(e) => onDropSatzChange(uebung.uiId, dsIdx, 'wiederholungen', e.target.value)}
                      />
                    </Grid>
                  </Grid>
                </Box>
...
function Trainingsergebnisse() {
  ...
  // ============================================================
  // GEÄNDERT: createDefaultErgebnisse statt createDefaultSaetze
  // Liefert jetzt immer { saetze: [...], dropsaetze: [] }
  // ============================================================
  const createDefaultErgebnisse = (count = 3) => ({
    saetze: Array.from({ length: count }, () => ({ wiederholungen: '', gewicht: '' })),
    dropsaetze: [],
  });

  // Rückwärtskompatibilität: Falls irgendwo noch plain arrays aus dem Cache kommen
  const normalizeErgebnis = (data) => {
    if (!data) return createDefaultErgebnisse();
    if (Array.isArray(data)) return { saetze: data, dropsaetze: [] }; // altes Format
    return {
      saetze: Array.isArray(data.saetze) ? data.saetze : [],
      dropsaetze: Array.isArray(data.dropsaetze) ? data.dropsaetze : [],
    };
  };
  ...
  useEffect(() => {
    ...
    if (isTimerRunning && timerStart) {
      interval = setInterval(() => {
        const diff = Math.floor((new Date().getTime() - new Date(timerStart).getTime()) / 1000);
        setElapsedTime(diff);
      }, 1000);
    }
  ...
  useEffect(() => {
    const fetchUebungen = async () => {
      ...
      try {
        ...
        const customRes = await TrainingApi.getUebungenByUserId(nutzerId);
      ...
      try {
        ...
        if (saved) {
          ...
          if (gewaehlte_uebungen && gewaehlte_uebungen.length > 0) {
            ...
            // GEÄNDERT: normalizeErgebnis stellt sicher, dass dropsaetze immer vorhanden ist
            const neueErgebnisse = {};
            uebungenMitNeuenIds.forEach(({ alte_uiId, neue_uiId }) => {
              const raw = savedErgebnisse?.[alte_uiId] || savedErgebnisse?.[neue_uiId];
              neueErgebnisse[neue_uiId] = normalizeErgebnis(raw);
            });
  ...
  const handlePlanChange = async (e, source) => {
    ...
    try {
      ...
      uebungenMitId.forEach(item => {
        initialErgebnisse[item.uiId] = createDefaultErgebnisse(item.empfohlene_saetze);
      });
  ...
  const handleDialogConfirm = () => {
    ...
    if (editIndex !== null) {
      ...
      // GEÄNDERT: createDefaultErgebnisse statt inline array
      setErgebnisse(prev => ({
        ...prev,
        [newItem.uiId]: createDefaultErgebnisse(3)
      }));
    }
    setDialogOpen(false);
  };
  ...
  // ── Normale Sätze ändern ──
  const handleInputChange = (uebungUiId, satzIdx, field, value) => {
    setErgebnisse(prev => {
      const existing = normalizeErgebnis(prev[uebungUiId]);
      return {
        ...prev,
        [uebungUiId]: {
          ...existing,
          saetze: existing.saetze.map((satz, idx) =>
  ...
  // ============================================================
  // NEU: Dropsatz-Handler
  // ============================================================
  const handleDropSatzAdd = (uebungUiId) => {
    setErgebnisse(prev => {
      const existing = normalizeErgebnis(prev[uebungUiId]);
      return {
        ...prev,
        [uebungUiId]: {
          ...existing,
          dropsaetze: [...existing.dropsaetze, { gewicht: '', wiederholungen: '' }]
        }
      };
    });
  };

  const handleDropSatzChange = (uebungUiId, dsIdx, field, value) => {
    setErgebnisse(prev => {
      const existing = normalizeErgebnis(prev[uebungUiId]);
      return {
        ...prev,
        [uebungUiId]: {
          ...existing,
          dropsaetze: existing.dropsaetze.map((ds, idx) =>
            idx === dsIdx ? { ...ds, [field]: value } : ds
          )
        }
      };
    });
  ...
  const handleDropSatzRemove = (uebungUiId, dsIdx) => {
    setErgebnisse(prev => {
      const existing = normalizeErgebnis(prev[uebungUiId]);
      return {
        ...prev,
        [uebungUiId]: {
          ...existing,
          dropsaetze: existing.dropsaetze.filter((_, idx) => idx !== dsIdx)
        }
      };
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.entries(ergebnisse).forEach(([uebungKey, data]) => {
        ...
        // Normale Sätze (ist_dropsatz = 0)
        normalized.saetze.forEach((satz, idx) => {
        ...
        // ============================================================
        // NEU: Dropsätze anhängen (ist_dropsatz = 1)
        // satz_nummer läuft nach den normalen Sätzen weiter
        // ============================================================
        const basisSatzNummer = normalized.saetze.length;
        normalized.dropsaetze.forEach((ds, idx) => {
          if (ds.wiederholungen && ds.gewicht) {
            alleErgebnisse.push({
              uebung_id: uebungId,
              satz_nummer: basisSatzNummer + idx + 1,
  ...
  const sortByName = (a, b) =>
    (a?.name || '').toString().localeCompare((b?.name || '').toString(), 'de', { sensitivity: 'base' });
  ...
  const TimerDisplay = () => {
    if (!isTimerRunning) return <TimerIcon />;
    return (
      <Typography sx={{ fontFamily: 'monospace', fontSize: '0.9rem', fontWeight: 700, color: '#3b82f6' }}>
    ...
    <>
      <ThemeProvider theme={darkTheme}>
                      ...
                      <FormControl fullWidth>
                        ...
                        >
                          ...
                          {customTrainingsplaene.map((customPlan, index) => (
                            <MenuItem key={`custom-${customPlan.id}-${index}`} value={customPlan.id}>
                              {customPlan.name}
                            </MenuItem>
                          ))}
                        </Select>
                      ...
                      <FormControl fullWidth>
                        ...
                        >
                          ...
                          {trainingsplaene.map((plan, index) => (
                            <MenuItem key={`standard-${plan.id}-${index}`} value={plan.id}>
                              {plan.name}
                            </MenuItem>
                          ))}
                        </Select>
                  ...
                  <Grid container spacing={2} sx={{ mb: 3 }}>
                    {(selectedCustomPlan || selectedStandardPlan) ? (
                      <>
                        ...
                        <Grid size={{ xs: 6, sm: 6 }}>
                          <Button variant="contained" size="large" fullWidth onClick={handleAddUebung} sx={{ height: '100%', fontSize: { xs: '0.85rem', sm: '1rem' }, padding: { xs: '10px 16px', sm: '14px 28px' }, fontWeight: 600, background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%)', color: '#fff', borderRadius: '16px', textTransform: 'none', transition: 'all 0.3s ease', '&:hover': { background: 'linear-gradient(135deg, #3b82f6 0%, #1e3a8a 100%)', boxShadow: '0 6px 18px rgba(30, 64, 175, 0.25)', transform: 'translateY(-2px)' } }}>
                            Übung hinz.
                          </Button>
                        </Grid>
                        <Grid size={{ xs: 6, sm: 6 }}>
                          <Button variant="contained" size="large" fullWidth onClick={() => navigate('/user/uebung-erstellen')} sx={{ height: '100%', fontSize: { xs: '0.85rem', sm: '1rem' }, padding: { xs: '10px 16px', sm: '14px 28px' }, fontWeight: 600, background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%)', color: '#fff', borderRadius: '16px', textTransform: 'none', transition: 'all 0.3s ease', '&:hover': { background: 'linear-gradient(135deg, #3b82f6 0%, #1e3a8a 100%)', boxShadow: '0 6px 18px rgba(30, 64, 175, 0.25)', transform: 'translateY(-2px)' } }}>
                            Übung erstellen
                          </Button>
                        </Grid>
                      </>
                  ...
                  {!isMobile && (
                    <Grid size={{ xs: 12, sm: 12 }} sx={{ pb: "15px" }}>
                      <Button
                        ...
                        sx={{ padding: { xs: '10px 16px', sm: '14px 28px' }, fontSize: { xs: '0.85rem', sm: '1rem' }, fontWeight: 600, borderRadius: '16px', background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%)', color: '#fff', textTransform: 'none', transition: 'all 0.3s ease', height: '100%', '&:hover': { background: 'linear-gradient(135deg, #3b82f6 0%, #1e3a8a 100%)', transform: 'translateY(-2px)', boxShadow: '0 8px 24px rgba(30, 64, 175, 0.3)' } }}
                      >
                        Training speichern
                      </Button>
                    </Grid>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth fullScreen={window.innerWidth < 600}>
              ...
              <DialogContent>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                  ...
                  <Autocomplete
                    options={[
                      ...gefilterteUebungen.map((u, index) => createExerciseItem({ id: `standard-${u.id}`, uebung_id: u.id, name: u.name, zielmuskel: u.zielmuskel, kategorie: u.kategorie, _quelle: "Übungen" }, 'standard', index)),
                      ...gefilterteCustomUebungen.map((u, index) => createExerciseItem({ id: `custom-${u.id}`, uebung_id: u.id, name: u.uebung_name, beschreibung: u.uebung_beschreibung, muskelgruppe: u.muskelgruppe, zielmuskel: u.zielmuskel, kategorie: u.kategorie, _quelle: "Meine Übungen" }, 'custom', index)),
                    ]}
                      ...
                      return (
                        <li key={key} {...other}>
                          <Box>
                            ...
                            <Typography variant="body2" color="text.secondary">{option.zielmuskel} • {option.kategorie}</Typography>
                          </Box>
                        </li>
                      );
                    }}
              ...
              <DialogActions>
                ...
                <Button onClick={handleDialogConfirm} variant="contained" disabled={!selectedUebung}>
        ...
        <Dialog open={saveConfirmOpen} onClose={() => setSaveConfirmOpen(false)}>
          ...
          <DialogContent>
            <Typography>Möchtest du dein Training wirklich speichern?</Typography>
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setSaveConfirmOpen(false)}>Abbrechen</Button>
            <Button onClick={handleSaveConfirm} variant="contained" sx={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%)', borderRadius: '8px', textTransform: 'none', fontWeight: 600 }}>
              Speichern
            </Button>
          </DialogActions>
        </Dialog>
